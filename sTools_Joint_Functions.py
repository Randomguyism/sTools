...
class JointsUI():
	...
	def jointSpliterButton(self, jointNumber, xyz, *args):
		jointNum = cmds.intSliderGrp(jointNumber, query = True, value = True)	# Finds the requested number of joints to insert
		jointAxis = cmds.radioButtonGrp(xyz, query = True, select = True)		# Indicates which axis joints are translated in
		attr = ['.tx', '.ty', '.tz']

		if jointAxis == 0:														# Errors if no axis is selected
			cmds.error('Joint translation axis not selected')
		elif jointAxis == 1:													# Checks which axis is selected and sends data to function
